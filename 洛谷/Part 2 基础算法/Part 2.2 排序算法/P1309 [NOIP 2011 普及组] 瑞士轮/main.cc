...
int grade[100000*2+10];
int weight[100000*2+10];
bool compareByGrade(const int& a,const int& b) {
    if(grade[a] != grade[b])
...
//    vector<vector<int>> arr1 = {{1,7,3},{9,8,2},{4,5,6}};
    ...
    ranges::stable_sort(pq1,compareByGrade);
    while (r--){
        int index = 0;
        while(index<pq1.size()){
            auto top1 = pq1[index];
            index++;
            auto top2 = pq1[index];
            index++;
            if(weight[top1] > weight[top2]){
                grade[top1]++;
            }else{
                grade[top2]++;
            }
        }
        ranges::stable_sort(pq1,compareByGrade); //ranges::stable_sort底层是归并排序，sort底层是快排
