...
class Solution {
   public:
    ...
    void dfs(unordered_map<string, double>& map1, string initialCurrency,
             double value,
             unordered_map<string, vector<pair<string, double>>>& graph) {
        map1[initialCurrency] = value;
        for (auto& [to, rate] : graph[initialCurrency]) {
            if (map1.find(to) == map1.end()) {
                dfs(map1, to, rate * value, graph);
            }
        }
    }

    unordered_map<string, double> calc_amount(vector<vector<string>>& pairs,
                                              vector<double>& rates,
        ...
        for (int i = 0; i < pairs.size(); i++) {
            auto& tmp = pairs[i];
            double rate = rates[i];
            graph[tmp[0]].emplace_back(tmp[1], rate);
            graph[tmp[1]].emplace_back(tmp[0], 1 / rate);
        }
        dfs(ans, initialCurrency, 1.0, graph);
        return ans;
    }
    double maxAmount(string initialCurrency, vector<vector<string>>& pairs1,
        ...
        unordered_map<string, double> arr1 =
            calc_amount(pairs1, rates1, initialCurrency);
        unordered_map<string, double> arr2 =
            calc_amount(pairs2, rates2, initialCurrency);
        double ans = 1;
        for (auto& [to, rate] : arr2) {
            ans = max(ans, arr1[to] / rate);
        }
        return ans;
    }
};
