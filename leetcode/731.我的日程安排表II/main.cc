...
public:
    ...
    map<int,int,MyCalendarTwo::Compare1> treeMap;
    MyCalendarTwo() {
    ...
    bool book(int startTime, int endTime) {
        int maxBook = 0;
        treeMap[startTime]++;//初值是0，相当于上车加人
        treeMap[endTime]--;//相当于下车减人
        for(const auto& iter:treeMap){
            int freq = iter.second;//出现频率
            maxBook += freq;
            if(maxBook>2){
                treeMap[startTime]--;
                treeMap[endTime]++;//还原现场
                return false;
            }
        }
        return true;
