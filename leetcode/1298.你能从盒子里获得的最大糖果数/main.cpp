...
class Solution {
   public:
    ...
    int maxCandies(vector<int>& status, vector<int>& candies,
        ...
        int ans = 0;
        queue<int> queue1;
        unordered_set<int> set1;  // 已经拥有的盒子,这个是最重要的部分。
        int n = status.size();
        ...
        while (!queue1.empty()) {
            ...
            queue1.pop();
            if (!isVisited[top] && status[top] == 1) {
                ans += candies[top];
                isVisited[top] = 1;
                for (int x : containedBoxes[top]) {
                    queue1.push(x);
                }
                for (int x : keys[top]) {
                    status[x] = 1;
                    if (!isVisited[x] && set1.find(x) != set1.end()) {
                        queue1.push(x);
                    }
                }
            } else {
...
signed main() {
    ...
    vector<vector<int>> arr1 = {{1, 2, 7}, {3, 6, 7}};
}
