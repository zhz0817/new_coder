...
class Solution {
   public:
    ...
    vector<int> minimumCost(int n, vector<vector<int>>& edges,
        ...
        UnionFind<int> uf(n);
        vector<int> ans;
        vector<int> and_values(n, -1);
        ans.reserve(query.size());  // 修改容量而非size，避免频繁扩容
        for (auto& edge : edges) {
            auto from = edge[0];
            auto to = edge[1];
            auto value = edge[2];
            ...
            and_values[y] &= value;
            if (x != y) {
                and_values[y] = and_values[x];
                uf.root[x] = y;  // 这里不能调用connect，因为用了and_values[y] =
                                 // and_values[x];，所以要保证y是祖先
            }
        }
        for (auto& tmp : query) {
            int s = tmp[0];
            int t = tmp[1];
            ans.push_back(uf.find(s) != uf.find(t) ? -1
                                                   : and_values[uf.find(s)]);
        }
        return ans;
    }
};
