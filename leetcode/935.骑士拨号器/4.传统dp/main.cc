...
public:
    ...
    // 同时考虑状态转移 0可以从4和6转移过来，类推
    int knightDialer(int n) {
        vector<vector<int>> dp(5005,vector<int>(15,0));
        int ans = 0;
        for(int i=0;i<n;i++){//手机号的长度
            for(int j=0;j<=9;j++){//手机号的最后一位
                if(i==0){
                    dp[i][j] = 1;//长度为0且最后一位是j的电话号只有一种
                }else{
                    for(int k=0;k<moves[j].size();k++){//考虑状态转移
