...
class Solution {
   public:
    ...
    vector<int> remainingMethods(int n, int k,
        ...
        for (auto& tmp : invocations) {
            graph[tmp[0]].push_back(tmp[1]);
        }
        vector<int> isQuestion(n, 0);
        ...
        for (auto& edges : invocations) {
            if (!isQuestion[edges[0]] && isQuestion[edges[1]]) {
                vector<int> ans(n, 0);
                iota(ans.begin(), ans.end(), 0);//返回一个长度为n，值为0，1，2，。。。n-1的vector
                return ans;
            }
        }
...
// 学习自 灵茶山艾府 https://leetcode.cn/u/endlesscheng/