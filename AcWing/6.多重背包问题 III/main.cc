...
int dp[N], pre[N], q[N];//单调递减的单调队列，q[N]存储背包容量索引k
int n, m;
signed main() {
  //    Solution s;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    int v, w, s;
    cin >> v >> w >> s;//体积 价值 数量
    for (int j = 0; j < v; ++j) {//从0开始，分成 v 个子问题，每个子问题处理背包容量 k 对应的一类情况。
      int head = 0, tail = -1;
      for (int k = j; k <= m; k += v) { //遍历背包容量，从 j 开始，以 v 为步长，处理背包的不同容量。每次处理的容量都是当前物品 v 的一个倍数

        if (head <= tail && k - s * v > q[head]) //检查队列头部元素是否已经过时，若过时，则弹出头部元素。
          ++head;

        while (head <= tail &&
               pre[q[tail]] - (q[tail] - j) / v * w <= pre[k] - (k - j) / v * w)
          --tail;//pre[q[tail]] - (q[tail] - j) / v * w 是物品从 q[tail] 转移到当前状态所能带来的价值增益。
          //pre[k] - (k - j) / v * w 是物品从当前容量 k 转移到下一个状态所能带来的价值增益。

        if (head <= tail)//队头大，和队头比
